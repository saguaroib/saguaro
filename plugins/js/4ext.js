 ...
 * All rights reserved.
 * Used under license https://github.com/4chan/4chan-JS/blob/master/LICENSE
*/
...
Parser.init = function() {
    ...
    staticPath = '//static.w4ch.org/css/imgs/';
...
Parser.getTrackedReplies = function(tid) {
    ...
    if (tracked = sessionStorage.getItem('ext-track-' + Main.board + '-' + tid)) {
...
Parser.buildPost = function(thread, board, pid) {
    ...
    for (i = 0; j = thread[i]; ++i) {
        ...
        if (Config.IDColor && IDColor.boards[board] && (uid = $.cls('posteruid', el)[1])) {
...
Parser.buildHTMLFromJSON = function(data, board) {
    ...
    noLink = data.resto + '#' + data.no;

    if (!Main.tid || board != Main.board) {
        noLink = 'res/' + noLink;
        quoteLink = 'res/' + data.resto + '#' + data.no;;
    } else {
    ...
    if (data.flag) {
        flag = '<span class="flag ' + data.flag + '"></span> ';
    }
    ...
    if (data.filedeleted) {
        ...
        if (data.spoiler) {
            ...
            if (!Config.revealSpoilers) {
                ...
                fileThumb = '//static.w4ch.org/css/imgs/spoiler' + (Parser.customSpoiler[board] || '') + '.png';
                data.tn_w = 100;
                data.tn_h = 100;
        ...
        if (!fileThumb) {
            fileThumb = '//t.w4ch.org/' + board + '/thumb/' + data.tim + 's.jpg';
        }
        ...
        fileInfo = '<span class="fileText" id="fT' + data.no + (data.spoiler ? ('" title="' + longFile + '"') : '"') + '>File: <a href="' + filePath + '" target="_blank">' + data.tim + data.ext + '</a>-(' + fileSize + 'B, ' + fileDims + (noFilename ? '' : (', <span title="' + longFile + '">' + shortFile + '</span>')) + ')</span>';

        fileBuildStart = fileInfo ? '<div class="fileInfo">' : '';
        fileBuildEnd = fileInfo ? '</div>' : '';

        fileHtml = '<div id="f' + data.no + '" class="file">' + fileBuildStart + fileInfo + fileBuildEnd + imgSrc + '</div>';
    }
    ...
    container.innerHTML =
        ...
        data.now + ' <a href="' + data.no + '#' + data.no + '">No.</a>' +
...
Parser.parseThread = function(tid, offset, limit) {
    ...
    UA.dispatchEvent('extParsingDone', {
        threadId: tid,
        offset: j,
        limit: limit
    });
...
Parser.parseMobileQuotelinks = function(post) {
    ...
    for (i = 0; link = quotelinks[i]; ++i) {
        t = link.getAttribute('href').match(/^(?:\/([^\/]+)\/)?(?:res\/)?([0-9]+)?#([0-9]+)$/);
...
Parser.parsePost = function(pid, tid) {
    ...
    if (tid) {
        ...
        if (Parser.needFile && (file = document.getElementById('fT' + pid))) {
            ...
            if (Config.downloadFile) {
                ...
                html +=
                    '<a href="' + file.firstElementChild.href + '" download="' + txt + '" title="Download file"><img class="extButton" src="' + Main.icons.download + '" alt="D"></a>';
            }

            if (Config.imageSearch) {
                href = file.firstElementChild.href;
                html +=
                    '<a href="//www.google.com/searchbyimage?image_url=' + href + '" target="_blank" title="Google Image Search"><img class="extButton" src="' + Main.icons.gis + '" alt="G"></a><a href="http://iqdb.org/?url=' + href + '" target="_blank" title="iqdb"><img class="extButton" src="' + Main.icons.iqdb + '" alt="I"></a>';
            }
    ...
    if (Config.revealSpoilers && (file = document.getElementById('f' + pid)) && (file = file.children[1])) {
        if ($.hasClass(file, 'imgspoiler')) {
            ...
            img.src = '//t.w4ch.org' + (file.pathname.replace(/src(\/[0-9]+).+$/, 'thumb$1s.jpg'))
            ...
            filename.lastChild.textContent = filename.lastChild.textContent.slice(0, -1) + ', ' + txt + ')';
...
Parser.parseBacklinks = function(pid, tid) {
    ...
    for (i = 0; j = backlinks[i]; ++i) {
        ...
        bl.innerHTML =
            '<a href="#' + pid + '" class="quotelink">&gt;&gt;' + pid + '</a> ';
...
QuoteInline.inlineRemote = function(link, board, tid, pid) {
    ...
    $.get('//api.w4ch.org/' + board + '/res/' + tid + '.json', {
        onload: onload,
        onerror: onerror
    });
};
...
QR.syncStorage = function(e) {
    ...
    if (key[0] == 'w4ch' && key[1] == 'cd' && e.newValue && Main.board == key[2]) {
...
QR.show = function(tid) {
    ...
    cnt.innerHTML =
        '<div id="qrHeader" class="drag postblock">Quick Reply - Thread No.<span id="qrTid">' + tid + '</span><img alt="X" src="' + Main.icons.cross + '" id="qrClose" ' + 'class="extButton" title="Close Window"></div>';
            ...
            if (placeholder == 'Password' || placeholder == 'Spoilers') {
                ...
                if (el.nodeName == 'INPUT' || el.nodeName == 'TEXTAREA') {
                    if (el.name == 'name') {
                        ...
                        if (cookie = Main.getCookie('w4ch_name')) {
                            el.value = cookie;
                        }
                        ...
                        if (cookie = Main.getCookie('w4ch_email')) {
                            el.value = cookie;
                        }
                    } else if (el.name == 'sub') {
    ...
    if (cd = localStorage.getItem('ext-cd-' + Main.board)) {
        QR.startCooldown(cd);
    }
...
QR.checkComLength = function() {
    ...
    if (QR.comLength) {
        ...
        if (byteLength > QR.comLength) {
            QR.showPostError('Comment too long (' + byteLength + '/' + QR.comLength + ')', 'length');
        } else {
...
QR.checkBan = function() {
    ...
    QR.banXhr.open('GET', '//api.w4ch.org/banned.php?' + Date.now(), true);
...
QR.submit = function(force) {
    ...
    QR.xhr.onload = function() {
        ...
        if (this.status == 200) {
            ...
            cd = Date.now() + '-' + cd;
            localStorage.setItem('ext-cd-' + Main.board, cd);
...
QR.purgeCooldown = function() {
    ...
    if (data = localStorage.getItem('ext-cd-' + Main.board)) {
        cd = data.split('-');
        ...
        if (elapsed >= thres || elapsed < 0) {
            localStorage.removeItem('ext-cd-' + Main.board);
        } else {
            return data;
        }
    }
...
ThreadHiding.load = function() {
    ...
    if (storage = localStorage.getItem('ext-hide-t-' + Main.board)) {
...
ThreadHiding.save = function() {
    for (var i in this.hidden) {
        localStorage.setItem('ext-hide-t-' + Main.board,
            JSON.stringify(this.hidden)
        );
        return;
    }
    localStorage.removeItem('ext-hide-t-' + Main.board);
};
...
ReplyHiding.load = function() {
    ...
    if (storage = localStorage.getItem('ext-hide-r-' + Main.board)) {
        this.hidden = JSON.parse(storage);
    }
};
...
ReplyHiding.save = function() {
    for (var i in this.hidden) {
        localStorage.setItem('ext-hide-r-' + Main.board,
            JSON.stringify(this.hidden)
        );
        return;
    }
    localStorage.removeItem('ext-hide-r-' + Main.board);
};
...
ThreadWatcher.syncStorage = function(e) {
    ...
    if (key[0] == 'w4ch' && key[1] == 'watch' && e.newValue != e.oldValue) {
...
ThreadWatcher.load = function() {
    if (storage = localStorage.getItem('ext-watch')) {
...
ThreadWatcher.build = function(rebuildButtons) {
    ...
    for (key in this.watched) {
        tuid = key.split('-');
        html += '<li id="watch-' + key + '"><span class="pointer" data-cmd="unwatch" data-id="' + tuid[0] + '" data-board="' + tuid[1] + '">&times;</span> <a href="' + Main.linkToThread(tuid[0], tuid[1], this.watched[key][1]) + '"';
...
ThreadWatcher.save = function() {
    localStorage.setItem('ext-watch', JSON.stringify(ThreadWatcher.watched));
};

ThreadWatcher.canAutoRefresh = function() {
    var time;

    if (time = localStorage.getItem('ext-tw-timestamp')) {
...
ThreadWatcher.setRefreshTimestamp = function() {
    localStorage.setItem('ext-tw-timestamp', Date.now());
};
...
ThreadWatcher.fetch = function(key, img) {
    ...
    xhr.open('GET', '//api.w4ch.org/' + tuid[1] + '/res/' + tuid[0] + '.json');
    xhr.send(null);
};
...
ThreadExpansion.expandComment = function(link) {
    ...
    if (!(ids = link.getAttribute('href').match(/^(?:res\/)([0-9]+)#([0-9]+)$/))) {
        return;
    }
    ...
    $.get('//api.w4ch.org/' + Main.board + '/res/' + tid + '.json', {
        onload: function() {
            var i, msg, com, posts;
...
ThreadUpdater.init = function() {
    ...
    this.iconPath = '//static.w4ch.org/css/imgs/';
    ...
    if (Config.updaterSound) {
       /* this.audioEnabled = false;
        ...
        this.audio.src = '//static.w4ch.org/media/beep.ogg';*/
    }
    ...
    if (sessionStorage.getItem('ext-auto-' + Main.tid)) {
        this.start();
    }
};
...
ThreadUpdater.start = function() {
    ...
    this.pulse();
    sessionStorage.setItem('ext-auto-' + Main.tid, 1);
};
...
ThreadUpdater.stop = function(manual) {
    ...
    sessionStorage.removeItem('ext-auto-' + Main.tid);
};
...
ThreadUpdater.onScroll = function(e) {
    if (document.documentElement.scrollHeight <= (window.innerHeight + window.pageYOffset) && !document[ThreadUpdater.hidden]) {
...
ThreadUpdater.update = function() {
    ...
    newTid = Main.tid.replace(".html","");
    $.get('//api.w4ch.org/' + Main.board + '/res/' + newTid + '.json', {
        onload: self.onload,
...
ThreadStats.updatePageNumber = function() {
    $.get('//api.w4ch.org/' + Main.board + '/threads.json', {
...
Filter.exec = function(cnt, pi, msg, tid) {
    ...
    for (i = 0; f = filters[i]; ++i) {
        if (f.type == 0) {
            ...
            if ((mail ||
                ((mail = pi.getElementsByClassName('useremail')[0]) && (mail = mail.href.slice(7)))
            ) && f.pattern.test(mail)) {
                hit = true;
                break;
            }
...
Filter.load = function() {
    ...
    if (!(rawFilters = localStorage.getItem('custom-filters'))) {
        return;
    }
...
<li>Type them exactly as they appear, including the exclamation mark for tripcode filters.</li>\
...
Filter.save = function() {
    ...
    if (rawFilters[0]) {
        localStorage.setItem('custom-filters', JSON.stringify(rawFilters));
    } else {
        localStorage.removeItem('custom-filters');
    }
};
...
CustomCSS.init = function() {
    var style, css;
    if (css = localStorage.getItem('custom-css')) {
        style = document.createElement('style');
        style.id = 'customCSS';
        style.setAttribute('type', 'text/css');
...
<textarea id="customCSSBox">' + (localStorage.getItem('custom-css') || '') + '</textarea>\
...
CustomCSS.save = function() {
    ...
    if (ta = $.id('customCSSBox')) {
        localStorage.setItem('custom-css', ta.value);
...
Report.onDone = function(e) {
    ...
    if (e.origin === 'https://dat.w4ch.org' && /^done-report/.test(e.data)) {
        if (Report.cnt) {
            Report.close();
        }
...
Report.openPopup = function(pid) {
    window.open('imgboard.php?mode=report&no=' + pid, Date.now(),
        "toolbar=0,scrollbars=0,location=0,status=1,menubar=0,resizable=1,width=600,height=250");
};
...
Report.openInline = function(pid) {
    ...
    this.cnt.innerHTML =
        '<div id="qrepHeader" class="drag postblock">Report Post No.' + pid + '<img alt="X" src="' + Main.icons.cross + '" id="qrepClose" ' + 'class="extButton" title="Close Window"></div>' + '<iframe src="imgboard.php?mode=report&no=' + pid + '" width="610" height="250" frameborder="0"></iframe>';
...
Config.load = function() {
    if (storage = localStorage.getItem('ext-settings')) {
...
Config.loadFromURL = function() {
    ...
    if (/#cfg$/.test(cmd[0])) {
        try {
            ...
            Config.save();
            if (data.filters) {
                localStorage.setItem('custom-filters', data.filters);
            }
            if (data.css) {
                localStorage.setItem('custom-css', data.css);
            }
            return true;
        } catch (e) {
            console.log(e);
...
Config.toURL = function() {
    ...
    cfg.settings = localStorage.getItem('ext-settings');

    if (data = localStorage.getItem('custom-filters')) {
        cfg.filters = data;
    }

    if (data = localStorage.getItem('custom-css')) {
        cfg.css = data;
    }
...
Config.save = function() {
    localStorage.setItem('ext-settings', JSON.stringify(Config));
};
...
SettingsMenu.options = {
    ...
    'Other': {
        ...
        localTime: ['Convert dates to local time', 'Convert server time (US Eastern Time) to your local time']
    }
};
...
<p class="center">[<a target="_blank" href="' + str + '">Restore Settings</a>]</p>';
...
Main.initGlobalMessage = function() {
    ...
    if ((msg = $.id('globalMessage')) && msg.textContent) {
        ...
        oldTs = localStorage.getItem('global-msg');
...
Main.toggleGlobalMessage = function() {
    ...
    if (msg.style.display == 'none') {
        ...
        btn.src = Main.icons.minus;
        btn.style.opacity = '1';
        localStorage.removeItem('global-msg');
    } else {
        msg.style.display = 'none';
        btn.src = Main.icons.plus;
        btn.style.opacity = '0.5';
        localStorage.setItem('global-msg', msg.getAttribute('data-utc'));
    }
};
...
Main.onclick = function(e) {
    ...
    if (cmd = t.getAttribute('data-cmd')) {
        ...
        if (QR.enabled && t.title == 'Quote') {
            e.preventDefault();
            console.log(t.previousElementSibling.getAttribute('resto'));
            tid = Main.tid || t.previousElementSibling.getAttribute('resto');
...
Main.onThreadMouseOver = function(e) {
    ...
    } else if (Config.imageHover && t.hasAttribute('md5') && !$.hasClass(t.parentNode, 'deleted')) {
        ImageHover.show(t);
...
Main.onThreadMouseOut = function(e) {
    ...
    } else if (Config.imageHover && t.hasAttribute('md5')) {
        ImageHover.hide();
    } else if (Config.filter && t.hasAttribute('data-filtered')) {
        QuotePreview.remove(t);
    }
...
Main.linkToThread = function(tid, board, post) {
    return '//' + location.host + '/' + (board || Main.board) + '/res/' + tid + (post > 0 ? ('#' + post) : '');
};
