        ...
        if (repod.suite_settings) {
            repod.suite_settings.info.push({
                menu: {
                    ...
                    variable: 'repod_quick_reply_enabled',
                    label: 'Quick reply',
            ...
            repod.suite_settings.info.push({
                menu: {
                    ...
                    variable: 'repod_quick_reply_persist',
            ...
            repod.suite_settings.info.push({
                menu: {
                    ...
                    variable: 'repod_quick_reply_autoreload',
                    label: 'Auto-reload without thread updater',
                    hover: 'Auto-reload the page after posting if thread updater is not enabled'
                }
            });
        }
        this.update();
    ...
    spawn_window: function(input) {
        ...
        qreply_clone.find("input").each(function() {
            ...
            if ($(this).attr('name') == 'num') {
                ...
                captcha.attr({
                    "src": repod.quick_reply.details.baseurl + "?" + new Date().getTime(),
                    "id": "qr_captcha"
                }).css({
                    "cursor": "pointer",
        ...
        qreply_clone.find("textarea").attr("id", "repod_jquery_quick_reply_textarea").removeAttr("cols").css({
            "width": "300px",
            "min-width": "300px"
        ...
        $("#repod_jquery_quick_reply_container_title > img#r_qr_close").on("click", function() {
            $("div#repod_jquery_quick_reply_container").remove();
        });
        if (jQuery.ui) {
            $("div#repod_jquery_quick_reply_container").draggable({
                handle: 'div#repod_jquery_quick_reply_container_title'
            });
        } //Bind jQuery UI draggable function.
        else {
            $("#repod_jquery_quick_reply_container").css({
                "right": "0px",
                "bottom": "100px",
                "top": ""
            });
        ...
        var options = {
            success: repod.quick_reply.callbacks.reply_success,
            resetForm: true,
            uploadProgress: repod.quick_reply.callbacks.upload_progress,
            beforeSubmit: repod.quick_reply.callbacks.beforeSubmit
        };
