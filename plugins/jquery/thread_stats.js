...
repod.suite_settings = {
    init: function() {
        ...
        $("a#repod_jquery_suite_settings_open").click(function() {
            repod.suite_settings.spawn.settings_window();
        });
    },
    spawn: {
        settings_window: function() {
            $("body").append(
                ...
                repod.suite_settings.config.width +
            ...
            $("#settings_window").on("click", function(event) {
                event.stopPropagation();
            });
            $("img#close").on("click", function() {
                $("div#settings_container").remove();
            });
        ...
        popup: function(popup_data) {
            if (popup_data["type"] !== "function") {
                ...
                $("#settings_popup_container").on("click", function() {
                    $(this).remove();
                });
                $("#settings_popup_window").on("click", function(event) {
                    event.stopPropagation();
                });
                $("#settings_popup_window").append(repod.suite_settings.populate.popup_footer(popup_data["type"]));
                $("#settings_popup_window > input[value='Save']").data(popup_data).on("click", function() {
                    repod.suite_settings.data_manip.save.popup(
                        $(this).data());
                });;
                $("#settings_popup_window > input[value='Reset']").data(popup_data).on("click", function() {
                    repod.suite_settings.data_manip.reset.popup(
                        $(this).data());
                });;
    ...
    populate: {
        settings_window: function() {
            $.each(repod.suite_settings.config.pre_categories, function(a, b) {
                repod.suite_settings.populate.spawn_category(b);
            });
            this.iterate(repod_suite_settings_pusher);
            this.iterate(repod.suite_settings.info.cache);
            $("#populated_settings > div > strong").on("click", function() {
                $("#" + $(this).text().replace(/[\A\W]/g, "-") + " > span").slideToggle();
            });
            $("#settings_window > strong").on("click", function() {
                if ($("#populated_settings > div > span").first().css("display") == "none") {
                    $("#populated_settings > div > span").slideDown();
                } else {
                    $
                        ("#populated_settings > div > span").slideUp();
                }
            });
            $("div.grouptoggle > a").on("click", function() {
                $("#" + $(this).parent().prev("strong").text().replace(/[\A\W]/g, "-") +
                    " > span > input[type='checkbox']").prop("checked", ($(this).text() == "On") ? "checked" : "");
            });
            $("#settings_window > input[value='Save']").on("click", function() {
                repod.suite_settings.data_manip.save.settings_window();
            });
            $("#settings_window > input[value='Reset']").on("click", function() {
                repod.suite_settings.data_manip.reset.settings_window();
            });
            $(".settings_popup").on("click", function() {
                repod.suite_settings.spawn.popup($(this).data());
            });
        },
        iterate: function(a) {
            $.each(a, function(a, b) {
                if (b['menu']) {
                    ...
                    if (cat && tvar && name) {
                        ...
                        if (b['menu']['read']) {
                            var c = (b['menu']['read'] === true) ? "checked" : ""
                        } else {
                            var c = (repod_jsuite_getCookie(tvar) === "true") ? "checked" : "";
                        }
                        ...
                        $("#populated_settings > #" + cat_safe + " > span").append("<input id='" + tvar + "' " + c +
                            " type='checkbox'><label for='" + tvar + "' title='" + desc + "'>" + name + "</label>" + popup + "<br />");
        ...
        popup: function(popup_data) {
            ...
            var output = "";
            if (popup_data['type'] == "textarea") {
                output = '<textarea id="' + popup_data["variable"] + '" placeholder="' + p + '" style="width:' + w +
                    'px">' + d + '</textarea>';
            }
            if (popup_data['type'] == "text") {
                output = '<input id="' + v + '" type="text" placeholder="' + p + '" value="' + d + '" style="width:' +
                    w + 'px"></input>';
            }
            if (popup_data['type'] == "info") {
                output = v;
            }
        ...
        popup_footer: function(type) {
            var o;
            if (type == "info") {
                o = "<input type='submit' value='Close'>";
            } else {
    ...
    data_manip: {
        save: {
            ...
            popup: function(popup_data) {
                ...
                if (c > 0 && popup_data['type'] == 'multitext') {
                    repod_jsuite_setCookie(popup_data['variable']['prefix'] + "amount", c, 7);
                }
