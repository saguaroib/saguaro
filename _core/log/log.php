...
class Log {
    ...
    function generate_all() {
        ...
        for ($page = 1; $page <= ceil(count($this->cache['THREADS']) / PAGE_DEF); $page++) {
            ...
            //Access thread cache generated by Index (for this instance) to generate Thread pages.
            foreach ($this->thread_cache as $no => $thread) {
                $logfilename = RES_DIR . $no . PHP_EXT;
                echo "Writing out $page#$no ($logfilename)... ";
                $temp = $this->generate("thread", $no, true);
