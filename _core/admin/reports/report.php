...
class SaguaroReports {
    ...
    private function canSubmit() {
        ...
        $query = "SELECT COUNT(no) FROM `" . SQLREPORTS . "` WHERE ip='$host' AND (board='{$board}' OR global='1')";
        ...
        $query = "SELECT COUNT(no) FROM `" . SQLREPORTS . "` WHERE ip='$host' AND no='{$no}' AND board='{$board}'";
        ...
        */
        ...
        if (($global && $type > 3) || (!$global && $type < 4)) {
            ...
            if ($mysql->result($query) >= 1) { //This isn't the first time the post has been reported.
                ...
                $bad = ['host', 'pwd', 'modified', 'last', 'sticky', 'permasage', 'root']; //Attributes to be stripped from encoded post.
