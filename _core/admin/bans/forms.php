...
class BanishForms {
    ...
    public function init() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
           ...
           //if (valid("janitor")) { //User is valid to ban request on this board.
              // if (valid("moderator")) { //User is valid to ban on this board.
            if ($_GET['a'] == 1)    {             
                return $this->banForm($no);
               } else {
                   return $this->requestForm();
               }
           /*} else { //User is not validated for this board.
                $this->error(S_NOPERM);
           }*/
        }
    }
    ...
    private function banForm($no) {
        ...
        $head = new Head;
		$head->info['page']['title'] = "/" . BOARD_DIR . "/ - Ban #{$no}";
        ...
        $temp .= "<!---banning #:$no;---><table class='banTable' border='0' cellpadding='0' cellspacing='0' /><form action='admin.php?mode=ban' method='POST' />
        ...
        $temp .= "<tr><td class='postblock'>History</td><td class='postHistory'>$alert</td></tr>
           <tr><td class='postblock'>Type</td><td>
            ...
            <tr><td class='postblock'>Append</td><td><input type='checkbox' name='showbanmess' title='Public ban, attaches this message to the post' /><input style='width:97%;'  type='text' name='custmess' placeholder='USER WAS BANNED FOR THIS POST' /></td></tr>
    ...
    private function requestForm() {
       ...
       /*if (!valid('janitor')) {
        ...
        $temp .= "<!---request on #:$no;---><table class='reqTable' border='0' cellpadding='0' cellspacing='0' /><form action='admin.php?mode=breq' method='POST' />
            <input type='hidden' name='no' value='$no' />";
        //$temp .= $csrf->field();
        $temp .= "<tr><td class='postblock'>Reason</td><td><textarea class='txtbox' type='text' name='reason' placeholder='Public reason for request' required/></textarea></td></tr>
            <tr><td class='postblock'>IP note</td><td><textarea class='txtbox' name='note' /></textarea></td></tr>
            <tr><td class='postblock'>After</td><td>
                <select name='after' />
                <option value='0' />None</option>
            ...
            <tr><td class='postblock'>Warn</td><td>[<input type='checkbox' name='warn' value='on'>Warn only?] <input  type='submit' value='Submit request' /></td></table></form>";
