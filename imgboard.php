...
//    if($lskd_fix_loop==0){echo "-->";} else {echo "";};
    ...
    if($ext){
      ...
      "\" border=\"0\" align=\"left\" class=\"postimg\" width=\"$w\" height=\"$h\" hspace=\"20\" alt=\"".$size." B\" /></a><br />";
        }else{
      ...
      "\" border=\"0\" align=\"left\" class=\"postimg\" hspace=\"20\" alt=\"".$size." B\" /></a><br />";
      }
...
if(USE_IMG_HOVER || USE_IMG_TOOLBAR || USE_IMG_EXP || USE_UTIL_QUOTE || USE_INF_SCROLL || USE_FORCE_WRAP || USE_UPDATER || USE_THREAD_STATS || USE_JS_SETTINGS || USE_EXTRAS){$dat.='<script src="'.JS_PATH.'/jquery.min.js" type="text/javascript"></script>';}
...
if(USE_EXTRAS){foreach(glob(JS_PATH."/extra/*.js") as $path){$dat.='<script src="'.$path.'" type="text/javascript"></script>';} unset($path);}
...
if(strlen($com) > S_POSTLENGTH) error(S_TOOLONG,$dest);
